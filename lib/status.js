...
StatusController.prototype.getSupply = function(req, res) {
  this.node.services.bitcoind.getSupply(function(err, result) {
      if (err) {
        return this.common.handleErrors(err, res);
      }      res.jsonp(result.supply);
    });
};

StatusController.prototype.getZeronodestats = function(req, res) {
  this.node.services.bitcoind.getZeronodestats(function(err, result) {
      if (err) {
        return this.common.handleErrors(err, res);
      }
      res.jsonp(result);
