...
function BlockController(options) {
  ...
  this.blockCacheConfirmations = 720;
...
BlockController.prototype.getBlockReward = function(height) {
  var subsidy = new BN(10 * 1e8);
   ...
   var halvings = Math.floor((height) / 800000);
  // Force block reward to zero when right shift is undefined.
  if (halvings >= 64) {
    return 0;
  }

  // Subsidy is cut in half every 800,000 blocks which will occur approximately every 4 years.*/
  subsidy >>= halvings;
